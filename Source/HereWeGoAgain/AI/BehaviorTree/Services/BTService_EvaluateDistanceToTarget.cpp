...
#include "BTService_EvaluateDistanceToTarget.h"
	...
	float DeltaSeconds)
{
	...
	if (bUsePathfinding)
	{
		...
		if (NavData)
		{
			...
			if (PathFindingResult.Result == ENavigationQueryResult::Type::Fail)
				UE_VLOG(OwnerComp.GetAIOwner(), LogAI, Warning, TEXT("BTService_EvaluateDistanceToTarget: Can't find path to target"));
		}
	}
	else
	{
	...
	if (OldDistance > 0.f)
	{
		UE_VLOG_CAPSULE(OwnerComp.GetAIOwner(), LogAI, VeryVerbose, TargetLocation - FVector::UpVector * 90, 90, 30, FQuat::Identity, FColor::White,
		...
		UE_VLOG(OwnerComp.GetAIOwner(), LogAI, VeryVerbose, TEXT("TargetMovedDistance = %.2f; AccumulatedTargetDeltaDistance = %.2f"), TargetMovedDistance,
