...
#include "AI/BehaviorTree/Decorators/Utility/BTDecorator_UtilityBlackboard.h"
#include "AI/BehaviorTree/Decorators/Utility/BTDecorator_UtilityFunction.h"
	...
	FBlackboard::FKey Key, uint8 ChildIndex)
{
	...
	UE_VLOG(AIController, LogAIUtility, VeryVerbose, TEXT("Utility changed for %s. Child index = %d. New utility value = %.2f"),
...
#if WITH_EDITOR
		UE_VLOG(AIController, LogAIUtility, Verbose, TEXT("Utility order changed:"));
		for (int i = 0; i < BTUtilityMemory->ActualUtilityNodesCount; i++)
		{
			UE_VLOG(AIController, LogAIUtility, Verbose,
